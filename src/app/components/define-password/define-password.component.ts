...
export class DefinePasswordComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.key !== null) {
      let dialog: IDialog;
      this.confirmService.confirmUser(this.key + "ADM").subscribe(response => {
        dialog = {
          title: 'Successful',
  ...
  onSubmit(cardForm): void {
    ...
    this.passwordService.changePassword(this.key, password).subscribe(response => {
      ...
      dialog = {
        title: 'Successful',
        description: 'Your password has been correctly reset',
        btnYes: 'Accept',
        type: TypeOfDialog.SUCCESS,
        icon: IconOfDialog.SUCCESS,
      ...
      dialog = {
        title: 'Error',
        description: 'Invalid key, You can use this link just once',
        btnNo: 'Accept',
